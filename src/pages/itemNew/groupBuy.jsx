...
import {timeCtrl, judgeIphoneX, addImageSuffix, getQueryString} from "src/js/util/index"
...
class FightGroupDetailList extends Component {
	...
	render() {
		...
		const comUcerterId = getQueryString('comUcerterId');
		const groupDetailUrl = comUcerterId ? `/groupDetail?comUcerterId=${comUcerterId}&`:`/groupDetail?`
		return (
			<li>
				<div className="list-detail">
					...
					<LinkChange className="list-right"
								to={user_id ? `${groupDetailUrl}object_id=${data.id}&user_id=${user_id}` : `${groupDetailUrl}object_id=${data.id}`}>
				...
				<ul className="ui-table-view">
					...
					<span className="posit-img"><img className="ui-media-object ui-pull-left"
						...
						<div className="ui-media-body window-head">
                            ...
                            <span className="c-c999">库存{retState.storeNum}</span>
